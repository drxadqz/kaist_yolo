...
def process_single_modal(modal_name):
    """
    处理单个模态（lwir/visible）的图片匹配、复制、重命名
    """
    print(f"\n{'=' * 80}")
    print(f"🚀 开始处理【{modal_name}】模态训练集")
    print(f"{'=' * 80}")

    # 路径拼接
    label_dir = os.path.join(LABEL_ROOT, modal_name, "train")
    target_img_dir = os.path.join(TARGET_IMG_ROOT, modal_name, "train")
    # 路径合法性校验
    ...
    # 3. 遍历所有标签文件，按顺序处理
    for label_file in tqdm(label_files, desc=f"处理{modal_name}图片"):
        # 拆解标签文件名
    ...
    # 保存错误日志
    log_dir = "kaist_dataset_fix_logs"
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f"{modal_name}_train_fix_log.txt")
    with open(log_file, 'w', encoding='utf-8') as f:
        f.write(f"===== {modal_name} 训练集图片匹配日志 =====\n")
...
def main():
    ...
    if total_missing == 0:
        print("🎉 所有模态处理完成！标签与图片已100%一一对应，可直接用于YOLO训练")
    else:
