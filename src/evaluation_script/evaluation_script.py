...
class CVCPedEval(KAISTPedEval):
    def __init__(self, kaistGt=None, kaistDt=None, iouType='segm', method='unknown'):
        ...
        self.params = CVCParams(iouType=iouType)   # parameters
        self.method = method
class CVCParams(Params):
    """Params for KAISTPed evaluation api"""
...
def evaluate(test_annotation_file: str, user_submission_file: str, day_night_split: int = 1455, plot=False):
    ...
    if day_night_split == 1455:
        EvalFunc = KAISTPedEval(kaistGt, kaistDt, 'bbox', method)
    elif day_night_split == 648:
        EvalFunc = CVCPedEval(kaistGt, kaistDt, 'bbox', method)
    else:
        raise NotImplementedError

    EvalFunc.params.catIds = [1]

    eval_result = {
        'all': copy.deepcopy(EvalFunc),
        'day': copy.deepcopy(EvalFunc),
        'night': copy.deepcopy(EvalFunc),
        'near': copy.deepcopy(EvalFunc),
        'medium': copy.deepcopy(EvalFunc),
        'far': copy.deepcopy(EvalFunc),
        'none': copy.deepcopy(EvalFunc),
        'partial': copy.deepcopy(EvalFunc),
        'heavy': copy.deepcopy(EvalFunc),
    }
    ...
    eval_result['day'].params.imgIds = imgIds[:day_night_split]
    eval_result['day'].evaluate(0)
    eval_result['day'].accumulate()
    MR_day = eval_result['day'].summarize(0)

    eval_result['night'].params.imgIds = imgIds[day_night_split:]
    eval_result['night'].evaluate(0)
    eval_result['night'].accumulate()
    MR_night = eval_result['night'].summarize(0)

    if day_night_split == 1455: # current dataset is vanilla KAIST: execute all protocol
    ...
    else: # current dataset is not KAIST: only execute All/Day/Night protocol
        MR_near, MR_medium, MR_far, MR_none, MR_partial, MR_heavy = 0., 0., 0., 0., 0., 0.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--annFile', type=str, default='src/evaluation_script/KAIST_annotation.json',
                        help='Please put the path of the annotation file. Only support json format.')
    parser.add_argument('--rstFiles', type=str, nargs='+', required=True,
                        help='Please put the path of the result file. Only support json, txt format.')
    parser.add_argument('--evalFig', type=str, default='runs/evaluation/KAIST_BENCHMARK.jpg',
                        help='Please put the output path of the Miss rate versus false positive per-image (FPPI) curve')
    args = parser.parse_args()

    phase = "Multispectral"
    results = [evaluate(args.annFile, result_file, phase) for result_file in args.rstFiles]
