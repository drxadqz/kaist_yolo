...
def _filter_invalid_yolo_labels(labels):
    if labels is None or len(labels) == 0:
        return labels, 0
    finite = np.isfinite(labels).all(axis=1)
    non_negative = (labels >= 0).all(axis=1)
    in_bounds = (labels[:, 1:] <= 1).all(axis=1)
    positive_wh = (labels[:, 3] > 1e-6) & (labels[:, 4] > 1e-6)
    keep = finite & non_negative & in_bounds & positive_wh
    return labels[keep], int((~keep).sum())


def _dataloader_worker_init_fn(worker_id):
    # Windows multiprocessing is more stable when each worker disables OpenCV threading/OpenCL explicitly.
    cv2.setNumThreads(0)
    try:
...
def create_dataloader_mono(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    windows_safe_loader = os.name == 'nt' and nw > 0
    loader = torch.utils.data.DataLoader if (image_weights or windows_safe_loader) else InfiniteDataLoader
    pin_memory = not windows_safe_loader
    if windows_safe_loader:
        logger.info(f'{prefix}Using Windows-safe standard DataLoader with {nw} worker(s)')
    ...
    dataloader = loader(dataset,
                        ...
                        worker_init_fn=_dataloader_worker_init_fn if nw > 0 else None,
...
def create_dataloader_rgb_ir(path1, path2,  imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        need_ignore_labels = bool(augment and (
            getattr(opt, 'kaist_ignore_aware_obj', False)
        ...
        dataset = LoadMultiModalImagesAndLabels(path1, path2, imgsz, batch_size,
                                                ...
                                                stride=int(stride),
                                                pad=pad,
                                                ignore_labels=need_ignore_labels,
                                                typed_aux_labels=bool(getattr(opt, 'kaist_typed_aux', False) and augment),
                                                typed_aux_root=getattr(opt, 'kaist_typed_annotation_root', None),
                                                protocol_aux_labels=bool(getattr(opt, 'kaist_protocol_aux', False) and augment),
                                                protocol_aux_root=(
                                                    getattr(opt, 'kaist_protocol_annotation_root', None)
                                                    or getattr(opt, 'kaist_typed_annotation_root', None)
                                                ),
                                                protocol_aux_config={
                                                    'height_thr': float(getattr(opt, 'kaist_protocol_height_thr', 55.0)),
                                                    'height_tau': float(getattr(opt, 'kaist_protocol_height_tau', 8.0)),
                                                    'heavy_occ_value': float(getattr(opt, 'kaist_protocol_heavy_occ_value', 0.25)),
                                                    'boundary': (
                                                        float(getattr(opt, 'kaist_protocol_boundary_left', 5.0)),
                                                        float(getattr(opt, 'kaist_protocol_boundary_top', 5.0)),
                                                        float(getattr(opt, 'kaist_protocol_boundary_right', 635.0)),
                                                        float(getattr(opt, 'kaist_protocol_boundary_bottom', 507.0)),
                                                    ),
                                                    'combine_mode': str(getattr(opt, 'kaist_protocol_validity_combine', 'min')),
                                                    'validity_floor': float(getattr(opt, 'kaist_protocol_validity_floor', 0.0)),
                                                },
                                                image_weights=image_weights,
                                                prefix=prefix)
...
def img2ignore_paths(img_paths):
    sb, t = 'labels_ignore', []
    for x in img_paths:
        if 'visible' in x.split(os.sep):
            sa = 'visible'
...
KAIST_TYPED_AUX_GROUP = 0
KAIST_TYPED_AUX_UNCERTAIN = 1
KAIST_TYPED_AUX_LABEL_MAP = {
    'people': KAIST_TYPED_AUX_GROUP,
    'person?': KAIST_TYPED_AUX_UNCERTAIN,
    'person?a': KAIST_TYPED_AUX_UNCERTAIN,
    'cyclist': KAIST_TYPED_AUX_UNCERTAIN,
}
KAIST_TYPED_AUX_IMG_WIDTH = 640.0
KAIST_TYPED_AUX_IMG_HEIGHT = 512.0
KAIST_PROTOCOL_AUX_IMG_WIDTH = 640.0
KAIST_PROTOCOL_AUX_IMG_HEIGHT = 512.0


def build_kaist_typed_aux_index(root):
    root = Path(root)
    index = {}
...
def parse_kaist_typed_aux_file(path):
    labels = []
    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
        for raw_line in f:
            ...
            if len(parts) < 5:
                continue
            aux_cls = KAIST_TYPED_AUX_LABEL_MAP.get(parts[0], None)
            if aux_cls is None:
                continue
            try:
            ...
            labels.append([
                float(aux_cls),
                min(max((x1 + w * 0.5) / KAIST_TYPED_AUX_IMG_WIDTH, 0.0), 1.0),
                min(max((y1 + h * 0.5) / KAIST_TYPED_AUX_IMG_HEIGHT, 0.0), 1.0),
                min(max(w / KAIST_TYPED_AUX_IMG_WIDTH, 0.0), 1.0),
                min(max(h / KAIST_TYPED_AUX_IMG_HEIGHT, 0.0), 1.0),
            ])
    if not labels:
...
def build_kaist_protocol_aux_index(root):
    root = Path(root)
    index = {}
    for path in root.rglob('*.txt'):
        index[path.stem] = path
...
def _clip_kaist_xywh(x1, y1, w, h, width=KAIST_PROTOCOL_AUX_IMG_WIDTH, height=KAIST_PROTOCOL_AUX_IMG_HEIGHT):
    x1c = min(max(float(x1), 0.0), width)
    y1c = min(max(float(y1), 0.0), height)
    x2c = min(max(float(x1) + float(w), 0.0), width)
    y2c = min(max(float(y1) + float(h), 0.0), height)
    return x1c, y1c, max(0.0, x2c - x1c), max(0.0, y2c - y1c)


def _soft_protocol_height_validity(height_value, threshold, tau):
    tau = max(float(tau), 1e-3)
    return 1.0 / (1.0 + math.exp(-(float(height_value) - float(threshold)) / tau))


def _protocol_boundary_validity(x1, y1, w, h, boundary):
    left, top, right, bottom = [float(v) for v in boundary]
    x2 = float(x1) + float(w)
    y2 = float(y1) + float(h)
    ix1 = max(float(x1), left)
    iy1 = max(float(y1), top)
    ix2 = min(x2, right)
    iy2 = min(y2, bottom)
    ...
    return min(max(inter / area, 0.0), 1.0)


def _combine_protocol_validity(height_validity, occ_validity, boundary_validity, combine_mode='min', validity_floor=0.0):
    components = [
        min(max(float(height_validity), 0.0), 1.0),
        min(max(float(occ_validity), 0.0), 1.0),
        min(max(float(boundary_validity), 0.0), 1.0),
    ]
    mode = str(combine_mode).strip().lower()
    if mode == 'product':
        validity = components[0] * components[1] * components[2]
    elif mode == 'geomean':
        validity = (max(components[0], 1e-6) * max(components[1], 1e-6) * max(components[2], 1e-6)) ** (1.0 / 3.0)
    else:
        validity = min(components)
    floor = min(max(float(validity_floor), 0.0), 1.0)
    validity = min(max(validity, floor), 1.0)
    return validity


def parse_kaist_protocol_aux_file(path, height_thr=55.0, height_tau=8.0, heavy_occ_value=0.25,
                                  boundary=(5.0, 5.0, 635.0, 507.0), combine_mode='min', validity_floor=0.0):
    labels = []
    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
        for raw_line in f:
            line = raw_line.strip()
            ...
            height_validity = _soft_protocol_height_validity(h, height_thr, height_tau)
            occ_validity = 1.0 if occ in (0, 1) else min(max(float(heavy_occ_value), 0.0), 1.0)
            boundary_validity = _protocol_boundary_validity(x1, y1, w, h, boundary)
            validity = _combine_protocol_validity(
                height_validity,
                occ_validity,
                boundary_validity,
            ...
            clip_x1, clip_y1, clip_w, clip_h = _clip_kaist_xywh(x1, y1, w, h)
            if clip_w <= 0.0 or clip_h <= 0.0:
                continue
            labels.append([
                float(validity),
                min(max((clip_x1 + clip_w * 0.5) / KAIST_PROTOCOL_AUX_IMG_WIDTH, 0.0), 1.0),
                min(max((clip_y1 + clip_h * 0.5) / KAIST_PROTOCOL_AUX_IMG_HEIGHT, 0.0), 1.0),
                min(max(clip_w / KAIST_PROTOCOL_AUX_IMG_WIDTH, 0.0), 1.0),
                min(max(clip_h / KAIST_PROTOCOL_AUX_IMG_HEIGHT, 0.0), 1.0),
            ])
    if not labels:
        return np.zeros((0, 5), dtype=np.float32)
    return np.array(labels, dtype=np.float32)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        l, dropped = _filter_invalid_yolo_labels(l)
                        if dropped:
                            print(f'{prefix}WARNING: Dropped {dropped} invalid non-positive/out-of-range labels from {lb_file}')
    ...
    def collate_fn(batch):
        if len(batch[0]) == 6:
            img, label, ignore, typed_aux, path, shapes = zip(*batch)
            for i, l in enumerate(label):
                l[:, 0] = i
            for i, l in enumerate(ignore):
                l[:, 0] = i
            for i, l in enumerate(typed_aux):
                l[:, 0] = i
            return torch.stack(img, 0), torch.cat(label, 0), torch.cat(ignore, 0), torch.cat(typed_aux, 0), path, shapes
        if len(batch[0]) == 5:
            img, label, ignore, path, shapes = zip(*batch)
            for i, l in enumerate(label):
                l[:, 0] = i
            for i, l in enumerate(ignore):
                l[:, 0] = i
            return torch.stack(img, 0), torch.cat(label, 0), torch.cat(ignore, 0), path, shapes
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, ignore_labels=False,
                 typed_aux_labels=False, typed_aux_root=None, protocol_aux_labels=False, protocol_aux_root=None,
                 protocol_aux_config=None, prefix=''):
        self.img_size = img_size
        self.augment = augment
        self.hyp = hyp
        self.image_weights = image_weights
        ...
        self.typed_aux_labels = bool(typed_aux_labels)
        self.typed_aux_root = Path(typed_aux_root) if typed_aux_root else None
        self.typed_aux_index = {}
        self.typed_aux_cache = {}
        self.protocol_aux_labels = bool(protocol_aux_labels)
        self.protocol_aux_root = Path(protocol_aux_root) if protocol_aux_root else None
        self.protocol_aux_index = {}
        self.protocol_aux_cache = {}
        self.protocol_aux_config = dict(protocol_aux_config or {})
        if self.typed_aux_labels:
            if self.typed_aux_root is None or not self.typed_aux_root.exists():
                raise FileNotFoundError(f'{prefix}typed aux annotation root not found: {self.typed_aux_root}')
            self.typed_aux_index = build_kaist_typed_aux_index(self.typed_aux_root)
        if self.protocol_aux_labels:
            if self.protocol_aux_root is None or not self.protocol_aux_root.exists():
                raise FileNotFoundError(f'{prefix}protocol aux annotation root not found: {self.protocol_aux_root}')
            self.protocol_aux_index = build_kaist_protocol_aux_index(self.protocol_aux_root)
        ...
        try:
            ...
            # self.img_files = sorted([x for x in f if x.suffix[1:].lower() in img_formats])  # pathlib
            assert self.img_files_rgb and self.img_files_ir, f'{prefix}No RGB/IR images found'
        except Exception as e:
        ...
        self.ignore_label_files_rgb = img2ignore_paths(self.img_files_rgb) if self.ignore_labels else []
        # print(self.label_files)
        ...
        if cache_rgb_path.is_file():
            try:
                cache_rgb = torch.load(cache_rgb_path, weights_only=False)
            except TypeError:
                cache_rgb = torch.load(cache_rgb_path)
            exists_rgb = True  # load
            if (
                cache_rgb['hash'] != get_hash(self.label_files_rgb + self.img_files_rgb)
                or cache_rgb.get('version') != CACHE_VERSION
            ):  # changed
        ...
        if cache_ir_path.is_file():
            try:
                cache_ir = torch.load(cache_ir_path, weights_only=False)
            except TypeError:
                cache_ir = torch.load(cache_ir_path)
            exists_ir = True  # load
            if (
                cache_ir['hash'] != get_hash(self.label_files_ir + self.img_files_ir)
                or cache_ir.get('version') != CACHE_VERSION
            ):  # changed
        ...
        self.ignore_label_files_rgb = img2ignore_paths(cache_rgb.keys()) if self.ignore_labels else []
        if single_cls:
        ...
        if self.typed_aux_labels:
            matched = sum(1 for img_path in self.img_files_rgb if Path(img_path).stem in self.typed_aux_index)
            logging.info(f"{prefix}Typed aux annotations matched: {matched}/{len(self.img_files_rgb)} from {self.typed_aux_root}")
        if self.protocol_aux_labels:
            matched = sum(1 for img_path in self.img_files_rgb if Path(img_path).stem in self.protocol_aux_index)
            logging.info(f"{prefix}Protocol aux annotations matched: {matched}/{len(self.img_files_rgb)} from {self.protocol_aux_root}")

    def load_ignore_labels(self, index):
        if not self.ignore_labels:
            return np.zeros((0, 5), dtype=np.float32)
        label_file = self.ignore_label_files_rgb[index]
    ...
    def load_typed_aux_labels(self, index):
        if not self.typed_aux_labels:
            return np.zeros((0, 5), dtype=np.float32)
        image_stem = Path(self.img_files_rgb[index]).stem
        label_file = self.typed_aux_index.get(image_stem, None)
        if label_file is None:
            return np.zeros((0, 5), dtype=np.float32)
        cache_key = str(label_file)
        if cache_key not in self.typed_aux_cache:
            self.typed_aux_cache[cache_key] = parse_kaist_typed_aux_file(label_file)
        labels = self.typed_aux_cache[cache_key]
        if labels.size == 0:
            return np.zeros((0, 5), dtype=np.float32)
        return labels.copy()

    def load_protocol_aux_labels(self, index):
        if not self.protocol_aux_labels:
            return np.zeros((0, 5), dtype=np.float32)
        image_stem = Path(self.img_files_rgb[index]).stem
        label_file = self.protocol_aux_index.get(image_stem, None)
        if label_file is None:
            return np.zeros((0, 5), dtype=np.float32)
        cache_key = str(label_file)
        if cache_key not in self.protocol_aux_cache:
            self.protocol_aux_cache[cache_key] = parse_kaist_protocol_aux_file(
                label_file,
                **self.protocol_aux_config,
            )
        labels = self.protocol_aux_cache[cache_key]
        if labels.size == 0:
            return np.zeros((0, 5), dtype=np.float32)
        return labels.copy()
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            ignore_labels = np.zeros((0, 5), dtype=np.float32)
            typed_aux_labels = np.zeros((0, 5), dtype=np.float32)
            protocol_aux_labels = np.zeros((0, 5), dtype=np.float32)
            ...
            ignore_labels = self.load_ignore_labels(index)
            typed_aux_labels = self.load_typed_aux_labels(index)
            protocol_aux_labels = self.load_protocol_aux_labels(index)
            if labels.size:  # normalized xywh to pixel xyxy format
                labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
            if ignore_labels.size:
                ignore_labels[:, 1:] = xywhn2xyxy(ignore_labels[:, 1:], ratio[0] * w, ratio[1] * h,
                                                   padw=pad[0], padh=pad[1])
            if typed_aux_labels.size:
                typed_aux_labels[:, 1:] = xywhn2xyxy(typed_aux_labels[:, 1:], ratio[0] * w, ratio[1] * h,
                                                     padw=pad[0], padh=pad[1])
            if protocol_aux_labels.size:
                protocol_aux_labels[:, 1:] = xywhn2xyxy(protocol_aux_labels[:, 1:], ratio[0] * w, ratio[1] * h,
                                                        padw=pad[0], padh=pad[1])
        ...
        nI = len(ignore_labels)
        if nI:
            ignore_labels[:, 1:5] = xyxy2xywh(ignore_labels[:, 1:5])
            ignore_labels[:, [2, 4]] /= img_rgb.shape[0]
            ignore_labels[:, [1, 3]] /= img_rgb.shape[1]
        nT = len(typed_aux_labels)
        if nT:
            typed_aux_labels[:, 1:5] = xyxy2xywh(typed_aux_labels[:, 1:5])
            typed_aux_labels[:, [2, 4]] /= img_rgb.shape[0]
            typed_aux_labels[:, [1, 3]] /= img_rgb.shape[1]
        nP = len(protocol_aux_labels)
        if nP:
            protocol_aux_labels[:, 1:5] = xyxy2xywh(protocol_aux_labels[:, 1:5])
            protocol_aux_labels[:, [2, 4]] /= img_rgb.shape[0]
            protocol_aux_labels[:, [1, 3]] /= img_rgb.shape[1]
        ...
        if self.augment:
            ...
            if random.random() < hyp['flipud']:
                ...
                if nI:
                    ignore_labels[:, 2] = 1 - ignore_labels[:, 2]
                if nT:
                    typed_aux_labels[:, 2] = 1 - typed_aux_labels[:, 2]
                if nP:
                    protocol_aux_labels[:, 2] = 1 - protocol_aux_labels[:, 2]
            ...
            if random.random() < hyp['fliplr']:
                ...
                if nI:
                    ignore_labels[:, 1] = 1 - ignore_labels[:, 1]
                if nT:
                    typed_aux_labels[:, 1] = 1 - typed_aux_labels[:, 1]
                if nP:
                    protocol_aux_labels[:, 1] = 1 - protocol_aux_labels[:, 1]

        labels_out = torch.zeros((nL, 6))
        if nL:
            labels_out[:, 1:] = torch.from_numpy(labels_rgb)
        ignore_out = torch.zeros((nI, 6))
        if nI:
            ignore_out[:, 1:] = torch.from_numpy(ignore_labels)
        typed_aux_out = torch.zeros((nT, 6))
        if nT:
            typed_aux_out[:, 1:] = torch.from_numpy(typed_aux_labels)
        protocol_aux_out = torch.zeros((nP, 6))
        if nP:
            protocol_aux_out[:, 1:] = torch.from_numpy(protocol_aux_labels)
        ...
        if self.typed_aux_labels or self.protocol_aux_labels:
            return torch.from_numpy(img_all), labels_out, ignore_out, typed_aux_out, protocol_aux_out, self.img_files_rgb[index], shapes
        if self.ignore_labels:
            return torch.from_numpy(img_all), labels_out, ignore_out, self.img_files_rgb[index], shapes
        return torch.from_numpy(img_all), labels_out, self.img_files_rgb[index], shapes

    @staticmethod
    def collate_fn(batch):
        if len(batch[0]) == 7:
            img, label, ignore, typed_aux, protocol_aux, path, shapes = zip(*batch)
            for i, l in enumerate(label):
                l[:, 0] = i
            for i, l in enumerate(ignore):
                l[:, 0] = i
            for i, l in enumerate(typed_aux):
                l[:, 0] = i
            for i, l in enumerate(protocol_aux):
                l[:, 0] = i
            return (
                torch.stack(img, 0),
                torch.cat(label, 0),
                torch.cat(ignore, 0),
                torch.cat(typed_aux, 0),
                torch.cat(protocol_aux, 0),
                path,
                shapes,
            )
        if len(batch[0]) == 6:
